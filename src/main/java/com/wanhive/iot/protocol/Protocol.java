...
public class Protocol extends FlowControl {
	 ...
	 * 
	 * @param message     The {@link Message} whose context will be checked
	 * @param context     The {@link MessageContext} to compare with
	 ...
	 * @return A {@link Message} carrying the identification request
	 */
	 ...
	 * 
	 * @param message The identification response {@link Message}
	 * @return An {@link IdentificationResponse} object
	 ...
	 * @return A {@link Message} carrying the authentication request
	 */
	 ...
	 * 
	 * @param message The authentication response {@link Message}
	 ...
	 * @return A {@link Message} carrying the registration request
	 */
	public Message createRegisterRequest(long uid, byte[] sid) {
		short length = Packet.HEADER_SIZE;
		Message message = new Message();
		if (sid != null) {
			message.payload().setBlob(0, sid);
			length += (short) sid.length;
		}
		MessageAddress address = new MessageAddress(uid, 0);
	 ...
	 * 
	 * @param message The registration response {@link Message}
	 ...
	 * 
	 * @param nonce 64-bit nonce generated by the client
	 * @return A {@link Message} carrying the session key request
	 */
	public Message createGetKeyRequest(byte[] nonce) {
		short length = Packet.HEADER_SIZE;
		Message message = new Message();
		if (nonce != null) {
			message.payload().setBlob(0, nonce);
			length += (short) nonce.length;
		}
		MessageAddress address = new MessageAddress();
		MessageControl ctrl = new MessageControl(length, nextSequenceNumber(), getSession());
	 ...
	 * 
	 * @param message The session key response {@link Message}
	 * @return 64-bytes long session key returned by the host
	 * @throws ProtocolException Request denied or invalid response
	 */
	 ...
	 * 
	 * @param uid Client's identity
	 * @return A {@link Message} carrying the bootstrap request
	 */
	 ...
	 * 
	 * @param message The bootstrap response {@link Message}
	 * @return The root host's identity
	 * @throws ProtocolException Request denied or invalid response
	 */
	 ...
	 * @param data  The bytes of data to be published
	 * @return A {@link Message} carrying the publish request
	 */
	public Message createPublishRequest(byte topic, byte[] data) {
		Message message = new Message();
		MessageAddress address = new MessageAddress();
		MessageControl ctrl = new MessageControl((short) (Packet.HEADER_SIZE + (data == null ? 0 : data.length)),
				nextSequenceNumber(), topic);
		message.setHeader(address, ctrl, RequestContext.PUBLISH);
		message.payload().setBlob(0, data);
		return message;
	}
	 ...
	 * 
	 * @param topic The topic identifier
	 * @return A {@link Message} containing the subscription request
	 */
	 ...
	 * 
	 * @param message The subscription response {@link Message}
	 ...
	 * @return A {@link Message} containing the unsubscription request
	 */
	 ...
	 * 
	 * @param message The unsubscription response {@link Message}
	 * @return The topic identifier
	 * @throws ProtocolException Request denied or invalid response
	 */
	 ...
	 * @return A {@link Message} carrying the request
	 */
